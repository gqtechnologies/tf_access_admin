<template>
  <nav class="w-full" :aria-label="t('admin.visits.new.steps_nav')">
...
import { VISIT_CREATE_STEPS } from '@/lib/schemas/visit_create'
...
const steps = computed(() =>
  VISIT_CREATE_STEPS.map((id) => ({
    id,
    label: t(`admin.visits.new.steps.${id}`),
  })),
)
