...
<template>
  ...
  >
    <SelectTrigger :id="props.id" class="w-[180px]" v-bind="triggerAttrs">
      <SelectValue :placeholder="t('admin.organizations.input.plan.placeholder')" />
    </SelectTrigger>
    <SelectContent>
      <SelectGroup>
        <SelectLabel>{{ t('admin.organizations.input.plan.label') }}</SelectLabel>
        <SelectItem v-for="plan in plans" :key="plan" :value="plan">
          {{ t(`plans.${plan}`) }}
        </SelectItem>
      </SelectGroup>
    </SelectContent>
