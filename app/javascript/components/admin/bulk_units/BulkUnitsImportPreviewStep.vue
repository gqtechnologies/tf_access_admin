  ...
  <div class="space-y-6">
    <div>
      <h3 class="text-base font-semibold">
        {{ t('admin.residential_properties.structure.bulk_import.preview.title') }}
      </h3>
      <p class="text-xs text-muted-foreground">
        {{ t('admin.residential_properties.structure.bulk_import.preview.subtitle') }}
      </p>
    </div>
      ...
      <div class="space-y-4">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div class="relative w-full sm:max-w-xs">
            ...
            <Input
              ...
              class="pl-9"
              :disabled="isLoading"
              :placeholder="t('admin.residential_properties.structure.bulk_import.preview.search_placeholder')"
            />
          </div>
        </div>
        ...
        <div class="overflow-hidden rounded-lg border">
          ...
          <Table v-else>
            <TableHeader>
              <TableRow>
                <TableHead class="w-16 text-xs">
                  {{ t('admin.residential_properties.structure.bulk_import.preview.table.row') }}
                </TableHead>
                <TableHead class="text-xs">
                  {{ t('admin.residential_properties.structure.bulk_import.preview.table.unit') }}
                </TableHead>
                <TableHead class="text-xs">
                  {{ t('admin.residential_properties.structure.bulk_import.preview.table.type') }}
                </TableHead>
                <TableHead class="text-xs">
                  {{ t('admin.residential_properties.structure.bulk_import.preview.table.status') }}
                </TableHead>
                <TableHead class="text-xs">
                  {{ t('admin.residential_properties.structure.bulk_import.preview.table.result') }}
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              <TableRow
                v-for="row in rows"
                :key="row.id"
                :class="rowTableClass(row)"
              >
                <TableCell class="text-sm tabular-nums text-muted-foreground">
                  {{ row.row_number }}
                </TableCell>
                <TableCell class="text-sm font-medium">
                  {{ row.normalized_payload.unit_identifier ?? '—' }}
                </TableCell>
                <TableCell class="text-sm">
                  {{ formatUnitType(row.normalized_payload.unit_type) }}
                </TableCell>
                <TableCell>
                  <Badge
                    :variant="validationStatusBadgeVariant(row.validation_status)"
                    class="text-xs"
                  >
        ...
        >
          ...
          <AlertDescription class="text-amber-900/90 dark:text-amber-100/90">
            {{ t('admin.residential_properties.structure.bulk_import.preview.attention_alert') }}
          </AlertDescription>
        </Alert>
      </div>
      ...
      <Card class="h-fit py-4">
        <CardContent class="space-y-4 px-4">
          <p class="text-sm font-medium">
            {{ t('admin.residential_properties.structure.bulk_import.preview.summary.title') }}
          </p>
          ...
          <ul class="space-y-3 text-sm">
            <li class="flex items-start gap-2">
              ...
              <span>
                {{
                  t('admin.residential_properties.structure.bulk_import.preview.summary.will_create', {
                    count: summary.valid,
                  })
                }}
              </span>
            ...
            <li
              v-if="summary.duplicates > 0 || summary.skipped > 0"
              class="flex items-start gap-2"
            >
              <TriangleAlert class="mt-0.5 size-4 shrink-0 text-amber-600" aria-hidden="true" />
              <span>
                {{
                  t('admin.residential_properties.structure.bulk_import.preview.summary.will_skip_duplicates', {
            ...
            <li v-if="summary.errors > 0" class="flex items-start gap-2">
              ...
              <span>
                {{
                  t('admin.residential_properties.structure.bulk_import.preview.summary.needs_correction', {
          ...
          <Alert class="border-sky-200 bg-sky-50 text-sky-950 dark:border-sky-900/50 dark:bg-sky-950/40 dark:text-sky-100">
            ...
            <AlertTitle>
              {{ t('admin.residential_properties.structure.bulk_import.preview.summary.tip_title') }}
            </AlertTitle>
...
import {
  useBulkUnitsPreviewRows,
  useBulkUnitsPreviewRowsQuery,
} from '@/lib/composables/bulk_units/useBulkUnitsPreviewRows'
import { primaryIssue } from '@/lib/utils/bulk_import_preview'
import type { BulkImportOwnerImportMode, BulkImportRecord, BulkImportRowsResponse } from '@/types/bulk_import'
...
const props = defineProps<{
  ...
  ownerImportMode: BulkImportOwnerImportMode
}>()
...
const residentialPropertyId = toRef(props, 'residentialPropertyId')
...
const {
  ...
  () => residentialPropertyId.value,
  () => bulkImportId.value,
)

const isLoading = computed(() => props.isValidating || isLoadingRows.value)

const { summary, showAttentionAlert, canContinueToImport, hasDownloadableErrors } =
  useBulkUnitsPreview(() => previewSummary.value)
...
watch(
  canContinueToImport,
  (value) => {
    emit('can-continue-change', value)
  },
  { immediate: true },
)
...
const previewFilters = computed(() =>
  BULK_IMPORT_PREVIEW_FILTERS.map((value) => ({
    value,
    label: t(`admin.residential_properties.structure.bulk_import.preview.filters.${value}`),
  })),
)
...
const summaryCards = computed(() => [
  {
    key: 'total',
    label: t('admin.residential_properties.structure.bulk_import.preview.cards.total'),
  ...
  {
    key: 'valid',
    label: t('admin.residential_properties.structure.bulk_import.preview.cards.valid'),
  ...
  {
    key: 'warnings',
    label: t('admin.residential_properties.structure.bulk_import.preview.cards.warnings'),
  ...
  {
    key: 'errors',
    label: t('admin.residential_properties.structure.bulk_import.preview.cards.errors'),
...
const emptyTableMessage = computed(() => {
  if (summary.value.total === 0) {
    return t('admin.residential_properties.structure.bulk_import.preview.empty.no_rows')
  }
  return t('admin.residential_properties.structure.bulk_import.preview.empty.no_matches')
})

const tipMessage = computed(() => {
  if (summary.value.errors > 0) {
    return t('admin.residential_properties.structure.bulk_import.preview.summary.tip_errors')
  }
  if (props.ownerImportMode === 'ignore') {
    return t('admin.residential_properties.structure.bulk_import.preview.summary.tip_owners_ignored')
  }
  if (props.ownerImportMode === 'link_existing') {
    return t('admin.residential_properties.structure.bulk_import.preview.summary.tip_owners_link_existing')
  }
  if (props.ownerImportMode === 'create_missing') {
    return t('admin.residential_properties.structure.bulk_import.preview.summary.tip_owners_create_missing')
  }
  return t('admin.residential_properties.structure.bulk_import.preview.summary.tip_default')
})

function formatUnitType(type: string | null | undefined) {
  if (!type) return '—'
  const key = `admin.residential_properties.structure.bulk_import.preview.unit_types.${type}`
  const translated = t(key)
  return translated === key ? type : translated
}

function formatValidationStatus(status: string) {
  const key = `admin.residential_properties.structure.bulk_import.preview.validation_statuses.${status}`
  const translated = t(key)
...
function resultLabel(row: (typeof rows.value)[number]) {
  ...
  const codeKey = `admin.residential_properties.structure.bulk_import.preview.results.${row.validation_status}`
