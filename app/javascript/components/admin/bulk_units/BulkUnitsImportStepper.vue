<template>
  <nav
    class="w-full"
    :aria-label="t('admin.residential_properties.structure.bulk_import.steps_nav')"
  >
...
import { BULK_UNITS_IMPORT_STEPS } from '@/lib/composables/bulk_units/useBulkUnitsImportDrawer'
...
const steps = computed(() =>
  BULK_UNITS_IMPORT_STEPS.map((id) => ({
    id,
    label: t(`admin.residential_properties.structure.bulk_import.steps.${id}`),
  }))
)
