<template>
  <nav class="w-full" :aria-label="t('admin.units.show.occupants.add_occupant.steps_nav')">
...
import type { UnitAddOccupantStepperStep } from '@/lib/composables/unit/useUnitAddOccupantDrawer'

const props = defineProps<{
  stepIndex: number
  visibleSteps: readonly UnitAddOccupantStepperStep[]
}>()
...
const steps = computed(() =>
  props.visibleSteps.map((id) => ({
    id,
    label: t(`admin.units.show.occupants.add_occupant.steps.${id}`),
  })),
)
