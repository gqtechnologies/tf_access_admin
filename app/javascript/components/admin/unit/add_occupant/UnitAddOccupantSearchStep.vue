  ...
  <div class="space-y-6">
    <div class="space-y-1">
      <h3 class="text-base font-semibold">
        {{ t('admin.units.show.occupants.add_occupant.search.title') }}
      </h3>
      <p class="text-sm text-muted-foreground">
        {{ t('admin.units.show.occupants.add_occupant.search.description') }}
      </p>
    </div>
    ...
    <div class="flex gap-2">
      <Input
        v-model="search"
        type="search"
        :placeholder="t('admin.units.show.occupants.add_occupant.search.placeholder')"
    ...
    <div class="space-y-3">
      <p class="text-sm font-medium">
        {{ t('admin.units.show.occupants.add_occupant.search.results_title') }}
      </p>

      <div v-if="loading" class="text-sm text-muted-foreground">
        {{ t('admin.units.show.occupants.add_occupant.search.loading') }}
      </div>
      ...
      >
        {{ t('admin.units.show.occupants.add_occupant.search.empty') }}
      </div>
    ...
    <div class="flex items-center justify-between gap-3 border-t pt-4">
      <p class="text-sm text-muted-foreground">
        {{ t('admin.units.show.occupants.add_occupant.search.not_found_prompt') }}
      </p>
      <Button type="button" variant="outline" @click="emit('create-person')">
        {{ t('admin.units.show.occupants.add_occupant.search.create_new') }}
      </Button>
    </div>
  </div>
</template>
