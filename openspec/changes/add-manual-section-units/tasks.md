...
- [ ] 1.3 Resolve submitted `property_section_id` through the current draft property and organization before invoking unit services.
- [ ] 1.4 Delegate individual and multiple manual unit creation to `Units::Create`, returning row-level errors for multiple creation without silently creating duplicates.
- [ ] 1.5 Delegate unit edits to `Units::Update` while permitting only `area_m2`, `display_name`, `unit_type`, and `identifier`.
- [ ] 1.6 Delegate unit deletion to `Units::SoftDelete` and ensure deleted units are excluded from subsequent wizard preview props.

## 2. Wizard Props and Preview Data

- [ ] 2.1 Extend property setup serialization/preview data to include non-deleted units grouped under their assigned sections for step 3.
- [ ] 2.2 Expose enough unit fields for preview rows and edit dialogs without leaking organization, property, lifecycle, or code mutation fields.
...
- [ ] 3.2 Build the add-unit dialog with individual and multiple modes, wired through VeeValidate and server error merging.
- [ ] 3.3 Build the unit edit dialog for `area_m2`, optional `display_name`, `unit_type`, and `identifier`.
- [ ] 3.4 Add unit delete confirmation using the existing confirm dialog pattern and send no request until confirmed.
- [ ] 3.5 Update `Step3Units.vue` to show manual section-level unit management when sections exist.

## 4. Shared Preview UI

- [ ] 4.1 Extend shared structure preview components to support a step 3 unit-management action mode.
...
- [ ] 6.1 Add focused controller/service tests for wizard-scoped manual unit create, multiple create, edit, and soft delete.
- [ ] 6.2 Add tests for tenant isolation, unauthorized access, foreign section/unit IDs, ineligible sections, duplicate identifiers, and soft-delete uniqueness release.
