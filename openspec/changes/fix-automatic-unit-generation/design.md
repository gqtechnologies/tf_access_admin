...
- No new database columns, migrations, or new service objects — this fixes existing services (`GenerateUnitsPreview`, `ApplyAutomaticUnits`, `BuildPreview`), one controller action/params method, and the frontend files identified above (`unitsPreview.ts`, `Step3Units.vue`, `usePropertySetupUnitsPreview.ts` if the param rename requires it, `Step2Structure.vue` only if it also reads `counts.towers`/`floors`).
- No change to `GenerateStructurePreview` or `ApplyQuickStructure` (step 2) — they already resolve leaves/levels correctly via `format.levels`; only step 3/4's units-and-summary side of the wizard had the hardcoded-type bug.
...
`@property.property_sections.where(section_type: format.units_in)` generalizes to `block` (condominium/horizontal/sector) and any future leaf type, instead of only supporting `floor`. Falls back to the existing flat/unsectioned behavior only when the property type has no recommended format (mirrors `ApplyQuickStructure`'s existing fallback pattern).
...
All three formats produce `units_per_leaf` units per leaf section (or per the flat fallback list when the property type has no recommended structure format); only the identifier numbering strategy changes per format.
...
Counts are keyed `level_1`/`level_2` (top level / leaf level), reusing the naming `GenerateStructurePreview.counts` already established, instead of inventing a new key scheme. `level_1` counts `sections.where(section_type: format.levels.first[:section_type])`; `level_2` counts `sections.where(section_type: format.units_in)`. When the property type has no recommended format, both are `0` (unchanged from today's behavior for those types). Alternative considered: keep the `towers`/`floors` key names but source them from the resolved format — rejected because a condominium's sectors/blocks would then be mislabeled as "towers"/"floors" in the JSON and in the UI copy, which is confusing even once the *count* is correct; renaming forces the UI copy to be fixed too instead of leaving a numerically-correct-but-mislabeled summary.
...
- [Risk] Renaming `BuildPreview.counts.towers/floors` → `counts.level_1/level_2` is a breaking shape change for any other consumer of that JSON besides `Step3Units.vue`. → Mitigation: task 9.4 explicitly greps for other consumers (`Step2Structure.vue`, `Step4Summary.vue`, etc.) before considering this done; no external/public API exposes this shape.
