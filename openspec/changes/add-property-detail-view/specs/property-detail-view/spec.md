...
The detail page SHALL show the relevant next-action options from the wizard step 5 confirmation/completion context in the "Próximos pasos recomendados" section, filtered by property status and user authorization. These are follow-up options, not step 5 save or confirmation controls.
