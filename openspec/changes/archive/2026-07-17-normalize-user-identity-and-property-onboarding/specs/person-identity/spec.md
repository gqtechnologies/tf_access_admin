...
Formalize the two-level identity model — `User` as the global authentication account and `Person` as the per-organization canonical identity — with explicit cardinality, ownership of global vs contextual data, and the rule that identity and account operations never implicitly create relationships to units.
