...
Defines how the system selects and applies a recommended `PropertyStructureFormat` based on a property's `property_type`. The format drives the structure wizard in step 2 (section level definitions, quick-form fields) and step 3 (unit identifier options, automatic generation availability).
